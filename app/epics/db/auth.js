...
export const storeGoogleAuthEpic = action$ => action$.pipe(
  ...
  mergeMap((action) => from(storeUser(action.payload.user)).pipe(
...
export const storeExchangeAuthEpic = action$ => action$.pipe(
  ofType(AuthActionTypes.SUCCESS_EXCHANGE_AUTH),
  mergeMap((action) => from(storeUser(action.payload.user)).pipe(
    mergeMap(resp => of(successStoreAuth(), retrieveStoreEvents(Providers.EXCHANGE))),
    catchError(error => {
      of(console.log(error));
    })
  )
