  ...
  switch(type) {
    ...
    case EXCHANGE:
      ...
      */
      ...
      [ 
        {"description": {"value": "Body", "defaultValue": "", "type": "optionalFunc", "func": function() { return dbEvent.Body.text; } }},
        {"location": {"value": "Location", "defaultValue": "", "type": "optional"}},
        {"htmlLink": {"value": "WebClientReadFormQueryString", "defaultValue": "", "type": "needed"}},
        {"allDay": {"value": "IsAllDayEvent", "defaultValue": false, "type": "needed"}},
        {"iCalUID": {"value": "ICalUid", "defaultValue": "", "type": "needed"}},
        {"created": {"value": "DateTimeCreated", "defaultValue": "", "type": "neededFunc", "func": function() { return dbEvent.DateTimeCreated.getMomentDate().format("YYYY-MM-DDTHH:mm:ssZ") } }},
        {"updated": {"value": "LastModifiedTime", "defaultValue": "", "type": "neededFunc", "func": function() { return dbEvent.LastModifiedTime.getMomentDate().format("YYYY-MM-DDTHH:mm:ssZ") } }},
        {"status": {"value": "IsCancelled", "defaultValue": "confirmed", "type": "neededFunc", "func": function() { return (dbEvent.IsCancelled === undefined || dbEvent.IsCancelled === null) ? 'confirmed' : 'cancelled' } }},
        {"organizer": {"value": "Organizer", "defaultValue": { email: "", displayName: "" }, "type": "neededFunc", "func": function() { return { email: dbEvent.Organizer.address, displayName: dbEvent.Organizer.name } } }},
          ...
          Object.keys(objMightHaveNothing).forEach(key => {
            ...
            if(objMightHaveNothing[key].type === "optional") {
              exchangeTryCatchCanBeNull(schemaCastedDbObject, key, dbEvent, objMightHaveNothing[key].value, objMightHaveNothing[key].defaultValue);
            } else if(objMightHaveNothing[key].type === "optionalFunc") {
              exchangeTryCatchCanBeNullFunc(schemaCastedDbObject, key, dbEvent, objMightHaveNothing[key].value, objMightHaveNothing[key].defaultValue, objMightHaveNothing[key].func);
            } else if(objMightHaveNothing[key].type === "needed") {
              exchangeTryCatchCannotBeNull(schemaCastedDbObject, key, dbEvent, objMightHaveNothing[key].value, objMightHaveNothing[key].defaultValue);
            } else if(objMightHaveNothing[key].type === "neededFunc") {
              exchangeTryCatchCannotBeNullFunc(schemaCastedDbObject, key, dbEvent, objMightHaveNothing[key].value, objMightHaveNothing[key].defaultValue, objMightHaveNothing[key].func);
            }
        }) 
      })
...
const exchangeTryCatchCanBeNull = (object, objectType, appointment, appointmentType, defaultValue) => {
  try {
...
const exchangeTryCatchCanBeNullFunc = (object, objectType, appointment, appointmentType, defaultValue, func) => {
  try {
    // console.log(objectType + ": " + appointment[appointmentType]);
    var value = func();
    if(value == null) {
...
const exchangeTryCatchCannotBeNull = (object, objectType, appointment, appointmentType, defaultValue) => {
  try {
...
const exchangeTryCatchCannotBeNullFunc = (object, objectType, appointment, appointmentType, defaultValue, func) => {
  try {
    var value = func();
    object[objectType] = value
  } catch (e) {
